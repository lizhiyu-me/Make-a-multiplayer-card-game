...
function dealCards_S2C(data) {
    ...
    var _myHandCardsShowArr = helper_1.convert2ReadableNames(mCardsArr);
...
function playCards_C2S() {
    ...
    console.log('Your cards->', helper_1.convert2ReadableNames(mCardsArr).join(','));
    ...
    if (_inputContent == "" || checkIsCardsLegal(_inputContent)) {
        var _cardsNumberArr = _inputContent == "" ? [] : helper_1.convert2CardNumbers(_inputContent.split(","));
...
function playCards_S2C(data) {
    ...
    if (_cardsPlayed.length == 0) {
        ...
        console.log("Player " + _seatNumber + "-> played " + helper_1.convert2ReadableNames(_cardsPlayed).join(",") + ".");
    }
}
